document.addEventListener('DOMContentLoaded', () => {
    ...
    nextBtn.addEventListener('click', () => {
        if (currentQuestionIndex < questions.length - 1) {
            ...
            // Update progress and redirect to the results/dashboard
            processCareerQuizResults(userResponses); // Pass responses to be processed
        }
    });

    // Process the user's answers and calculate scores
    function processCareerQuizResults(userResponses) {
        // Map answers to categories (e.g., A=STEM, B=Trades, C=Leadership, D=Creative)
        const scoringGuide = {
            0: 'STEM', // A in first position
            1: 'Trades', // B in second position
            2: 'Leadership', // C in third position
            3: 'Creative' // D in fourth position
        };
        ...
        console.log(categoryScores); // Debugging - check the scores

        // Store the scores or redirect to the results page
        location.href = 'dashboard.html'; // Change to dashboard to show results
    }
});
